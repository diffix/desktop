...
By default, __Diffix__ suppresses bins with fewer than 5 protected entities *on average*. This corresponds to the `Suppression Threshold` anonymization parameter equal 3, with the average modification added. This can be configured (see section [Configure anonymization parameters](operation.md#configure-anonymization-parameters)). A minimum value 2 of `Suppression Threshold` is enforced, therefore __Diffix__ always suppresses bins with only a single contributing protected entity.
